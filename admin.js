...
// Mock Booking Data
const bookings = [
    { id: '#BK001', guest: 'Budi Santoso', room: 'Deluxe Garden View', checkIn: '15 Nov', checkOut: '17 Nov', total: 2000000, status: 'Terkonfirmasi' },
    { id: '#BK002', guest: 'Siti Aminah', room: 'Premium Villa', checkIn: '20 Nov', checkOut: '23 Nov', total: 6000000, status: 'Pending' },
    { id: '#BK003', guest: 'John Doe', room: 'Standard Room', checkIn: '01 Des', checkOut: '05 Des', total: 1000000, status: 'Terkonfirmasi' },
];
                ...
                <tbody>
                    ${bookings.map(b => `
                        <tr>
                            ...
                            <td>${b.checkIn}</td>
                            <td>${b.checkOut}</td>
                            <td style="color: var(--primary); font-weight: bold;">Rp ${b.total.toLocaleString('id-ID')}</td>
                            <td>
                                <span class="status-badge ${b.status === 'Terkonfirmasi' ? 'status-success' : 'status-warning'}">
                                    ${b.status}
                                </span>
                            </td>
                            <td>
                                <div class="flex gap-2">
                                    ...
                                    ${b.status === 'Pending' ? `
                                        <button class="btn btn-primary" style="padding: 4px 8px; background: #10b981;" onclick="window.approveBooking('${b.id}')">
            ...
            ${rooms.map(room => `
                <div class="dashboard-card" style="padding: 20px;">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h3 style="font-size: 18px; margin-bottom: 5px;">${room.name}</h3>
                            <span class="status-badge ${room.available > 0 ? 'status-success' : 'status-danger'}">
                    ...
                    <div style="font-weight: bold; color: var(--primary); margin-bottom: 15px;">
                        Rp ${room.price.toLocaleString('id-ID')} <span style="font-weight: normal; color: var(--text-light); font-size: 12px;">/ malam</span>
                    </div>

                    <div style="width: 100%; background: #f3f4f6; height: 6px; border-radius: 3px; overflow: hidden;">
                        <div style="width: ${(room.available / 5) * 100}%; background: var(--primary); height: 100%;"></div>
                    </div>
                    <div class="flex justify-between text-xs text-gray-400 mt-1">
                        <span>Tingkat Ketersediaan</span>
                        <span>${(room.available / 5) * 100}%</span>
                    </div>
                </div>
            `).join('')}
        </div>
...
document.querySelectorAll('.nav-tab').forEach(tab => {
    tab.addEventListener('click', () => {
        ...
        if (tabName === 'pemesanan') renderPemesanan();
        else if (tabName === 'kamar') renderKamar();
        else document.getElementById('adminContent').innerHTML = '<div class="text-center p-10">Fitur belum tersedia</div>';
    });
});
